...
Based on the code 'plot_subjet_raa.py' by James Mulligan
'''
...
class PlotRAA(common_base.CommonBase):
  ...
  def initialize(self):
    ...
    with open(config_file, 'r') as stream:
      ...
      self.file_jetscape_pp_name = None
      self.file_jetscape_AA_name = None

      if 'jetscape_pp' in config:
        self.file_jetscape_pp_name = config['jetscape_pp']

      if 'jetscape_AA' in config:
  ...
  def init_result(self, result):
    ...
    integral_AA = self.h_main_AA.Integral(1, self.n_bins+1, 'width')
    self.h_main_AA.Scale(1./integral_AA)
    self.h_sys_AA.Scale(1./integral_AA)
  
    integral_pp = self.h_main_pp.Integral(1, self.n_bins+1, 'width')
  ...
  def plot_result(self):
    ...
    myLegend = ROOT.TLegend(0.70,0.65,0.87,0.85)
    ...
    text_latex.DrawLatex(x, 0.83, '#bf{{ALICE}} {}'.format(self.figure_approval_status))
    text_latex.DrawLatex(x, 0.75, '#sqrt{#it{s_{#it{NN}}}} = 5.02 TeV')
    text_latex.DrawLatex(x, 0.67, 'Charged jets   anti-#it{k}_{T}')
    text_latex.DrawLatex(x, 0.6, '#it{R} = ' + str(self.jetR) + '   |#it{{#eta}}_{{jet}}| < {}'.format(0.9-self.jetR))
    text_latex.DrawLatex(x, 0.51, str(self.min_pt) + ' < #it{p}_{T, ch jet} < ' + str(self.max_pt) + ' GeV/#it{c}')
    text_latex.DrawLatex(x, 0.41, str(self.obs_label))
  ...
  def init_theory(self, result):
    print('GOTTA IMPLEMENT init_theory')
    exit()
    ''' 
    self.label_list = []
    self.sublabel_list = []
