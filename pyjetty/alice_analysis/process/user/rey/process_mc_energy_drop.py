...
################################################################
class ProcessMC_energy_drop(process_mc_base.ProcessMCBase):
  ...
  def __init__(self, input_file='', config_file='', output_dir='', debug_level=0, **kwargs):
    ...
    super(ProcessMC_energy_drop, self).__init__(input_file, config_file, output_dir, debug_level, **kwargs)
  ...
  def initialize_user_output_objects_R(self, jetR):
        ...
        if observable == 'energy_drop':
          ...
          for grooming_setting in self.obs_grooming_settings[observable]:
            if grooming_setting:
              ...
              if self.is_pp:
                self.create_energy_drop_histograms(observable, jetR, grooming_label)
              else:
                for R_max in self.max_distance:
                  self.create_energy_drop_histograms(observable, jetR, grooming_label, R_max)
                  if R_max == self.main_R_max:
                    self.create_energy_drop_histograms(observable, jetR, grooming_label, '{}_matched'.format(R_max))
      ...
      for grooming_setting in self.obs_grooming_settings['energy_drop']:
        if grooming_setting:
          grooming_label = self.utils.grooming_label(grooming_setting)
          if not self.is_pp:
  ...
  #---------------------------------------------------------------
  # Create energy_drop response histograms
  #---------------------------------------------------------------
  ...
  # Create energy_drop response histograms
  #---------------------------------------------------------------
  def create_energy_drop_histograms(self, observable, jetR, grooming_label, R_max = None):
    ...
    # Create THn of response for energy_drop
    if self.fill_RM_histograms:
      dim = 4;
      title = ['p_{T,det}', 'p_{T,truth}', 'JED_{det}', 'JED_{g,truth}']
      nbins = [30, 20, 100, 100]
      min = [0., 0., 0., 0.]
      max = [150., 200., 1., 1.]
    ...
    h.GetXaxis().SetTitle('p_{T,truth}')
    h.GetYaxis().SetTitle('JED_{truth}')
    h.GetZaxis().SetTitle('#frac{JED_{det}-JED_{truth}}{JED_{truth}}')
    setattr(self, name, h)
  ...
  def fill_observable_histograms(self, hname, jet, jet_groomed_lund, jetR, obs_setting,
      ...
      getattr(self, hname.format('energy_drop', obs_label)).Fill(jet_pt_ungroomed, jet_energy_drop)
  ...
  def fill_matched_jet_histograms(self, jet_det, jet_det_groomed_lund, jet_truth,
    ...
    jet_energy_drop_det = (jet_det.pt()-jet_groomed_det.pt())/jet_det.pt()

    jet_groomed_truth = jet_truth_groomed_lund.pair()
    jet_energy_drop_truth = (jet_truth.pt()-jet_groomed_truth.pt())/jet_truth.pt()
    ...
    if not self.is_pp and grooming_setting in self.obs_grooming_settings['energy_drop']:
    ...
    # Fill histograms
    observable = 'energy_drop'
    self.fill_response(observable, jetR, jet_pt_det_ungroomed, jet_pt_truth_ungroomed, jet_energy_drop_det, jet_energy_drop_truth, obs_label, R_max, prong_match = prong_match)
...
if __name__ == '__main__':
  ...
  analysis = ProcessMC_energy_drop(input_file=args.inputFile, config_file=args.configFile, output_dir=args.outputDir)
  analysis.process_mc()
